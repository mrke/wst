...
TC_f=0.2779; % temp correction factor when in water
TC_0=1.1317; % temp correction factor when on land
t=1:(365*years); % vector of days to simulate
eLHR0 = [f; L_b; g * s_Megg * u_Hb/ l_b^3; 0]; % initial scaled reserve, length, scaled maturity density and reproduction buffer (birth)
pars_UE0 = [V_Hb; g * s_Megg; k_J * s_Megg; k_M * s_Megg; v * s_Megg]; U_E0 = initial_scaled_reserve(0.8, pars_UE0);
...
days=(1:112);
act=ones(1,112);
tf=vertcat(days,act)'; % 112 days in the water
TC_f=0.5212;
TC_0=1.36442;
t=days;                     % vector of days to simulate
eLHR0 = [f; L_b; g * s_M * u_Hb/ l_b^3; 0]; % initial scaled reserve, length, scaled maturity density and reproduction buffer (birth)
pars_UE0 = [V_Hb; g * s_M; k_J * s_M; k_M * s_M; v * s_M]; U_E0 = initial_scaled_reserve(0.8, pars_UE0);
[t eLHR] = ode45(@dget_eLHR, t, eLHR0, [], s_M, kap, u_Hp, k_J, v, g, L_m, f, TC_f, TC_0, tf);
e = eLHR(:,1); L = eLHR(:,2); H = eLHR(:,3); eR = eLHR(:,4); teR = [t, eR];
