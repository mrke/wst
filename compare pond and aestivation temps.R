...
microin<-"pond and aestivation" # subfolder containing the microclimate input data
...
minshade<-70. # minimum available shade (percent)
...
conth<-40 # cylindrical container/pond height (cm)
contw<-200. # cylindrical container/pond diameter (cm)
...
continit<-40 # initial container water level (cm)
...
daystart<-'11/02/01' # y/m/d
dayfin<-'11/02/30' # y/m/d
plotpred<-subset(environ, format(environ$dates, "%y/%m/%d")>= daystart & format(environ$dates, "%y/%m/%d")<=dayfin)
with(plotpred, plot(WATERTEMP~dates,type = "l"))

path1<-"pond and aestivation/"

Hatchlings<-read.csv(paste(path1,'Hatchlings.csv',sep=""))
Hatchlings$date<-as.POSIXct(Hatchlings$date,tz=tzone,format="%d/%m/%Y")
Hatchlings$datemin<-as.POSIXct(Hatchlings$datemin,tz=tzone,format="%d/%m/%Y %H")-3600
Hatchlings$datemax<-as.POSIXct(Hatchlings$datemax,tz=tzone,format="%d/%m/%Y %H")-3600+3*3600
Hatch<-cbind(as.data.frame(c(Hatchlings$datemin,Hatchlings$datemax)),as.data.frame(c(Hatchlings$min,Hatchlings$max)))
colnames(Hatch)<-c("date","temp")
Hatch<-Hatch[order(Hatch$date),]

points(Hatch$temp~Hatch$date,type='l',col='red')


with(environ, points(TA~dates,type = "l",col='light blue'))
